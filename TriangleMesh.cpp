...
void TriangleMesh::translateToCenter(const Vec3f& newBBmid, const bool createVBOs) {
	...
	// data changed => delete VBOs and create new ones
...
void TriangleMesh::scaleToLength(const float newLength, const bool createVBOs) {
...
void TriangleMesh::loadOFF(const char* filename, const bool createVBOs) {
...
void TriangleMesh::checkVBOSupport() {
	...
	if (glGenBuffers && glBindBuffer && glBufferData && glBufferSubData && glMapBuffer && glUnmapBuffer && glDeleteBuffers && glGetBufferParameteriv) VBOsupported = true;
	else VBOsupported = false;
}

GLuint TriangleMesh::createVBO(const void* data, const int dataSize, const GLenum target, const GLenum usage) {
...
void TriangleMesh::setDrawMode(const unsigned int _drawMode) {
	drawMode = _drawMode;
...
unsigned int TriangleMesh::draw() {
	...
	case 1:
		...
		break;
	case 2:
		if (VBOsupported == true) return drawVBO();
		else return drawArray();
		break;
	default:
...
unsigned int TriangleMesh::drawImmediate() {
	if (triangles.size() == 0) return 0;
	if (withTexture && texCoords.size() == vertices.size()) glEnable(GL_TEXTURE_2D);
	if (withColorArray && colors.size() == vertices.size()) glEnable(GL_COLOR_MATERIAL);
	...
	if (withColorArray && colors.size() == vertices.size()) glDisable(GL_COLOR_MATERIAL);
	return triangles.size();
}

unsigned int TriangleMesh::drawArray() {
	if (triangles.size() == 0) return 0;
...
unsigned int TriangleMesh::drawVBO() {
	if (VBOf == 0 || VBOv == 0 || VBOn == 0) return 0;
	glEnableClientState(GL_NORMAL_ARRAY);
	glEnableClientState(GL_VERTEX_ARRAY);
