...
Vec3f PhongIllumination::IlluminationCalculation(SceneObject& sceneObject, vector <light>& lightSources,
	...
	float objectReflection = sceneObject.reflectivity;
	float objectTransparency = sceneObject.refraction;
	...
	for (unsigned int i = 0; i < 3; i++) {
		...
		DS_sumOverLights[i] += objectReflection * RecursiveRayIntensity[i];
		DS_sumOverLights[i] += objectTransparency * transparencyIntensity[i];
	}
